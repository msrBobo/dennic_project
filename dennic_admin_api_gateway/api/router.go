...
// NewRoute
// @title Dennic Admin Project
// @version 1.7
// @host admin.dennic.uz
...
func NewRoute(option RouteOption) *gin.Engine {
	...
	// Authorization
	authorization := api.Group("/admin")
	authorization.POST("/forget-password", HandlerV1.ForgetPassword)
	authorization.PUT("/update-password", HandlerV1.UpdatePassword)
	authorization.POST("/verify-otp-code", HandlerV1.VerifyOtpCode)
	authorization.POST("/send-otp", HandlerV1.SenOtpCode)
	authorization.POST("/login", HandlerV1.Login)

	// user
	user := api.Group("/user")
	user.POST("/create", HandlerV1.CreateUser)
	...
	admin.POST("/create", HandlerV1.CreateAdmin)
	admin.GET("/get", HandlerV1.GetAdminByID)
	admin.GET("/", HandlerV1.ListAdmins)
	admin.PUT("/", HandlerV1.UpdateAdmin)
	admin.PUT("/update-admin-refresh-token", HandlerV1.UpdateAdminRefreshToken)
	admin.DELETE("/", HandlerV1.DeleteAdmin)
	...
	doctorNote.POST("/create", HandlerV1.CreateDoctorNote)
	...
	appointment.POST("/create", HandlerV1.CreateBookedAppointment)
	...
	doctorTime.POST("/create", HandlerV1.CreateDoctorTimes)
	...
	patient.POST("/create", HandlerV1.CreatePatient)
	...
	department.POST("/create", HandlerV1.CreateDepartment)
	...
	doctor.POST("/create", HandlerV1.CreateDoctor)
	...
	specialization.POST("/create", HandlerV1.CreateSpecialization)
	...
	doctorServices.POST("/create", HandlerV1.CreateDoctorService)
	...
	doctorWorkingHours.POST("/create", HandlerV1.CreateDoctorWorkingHours)
	...
	reasons.POST("/create", HandlerV1.CreateReasons)
