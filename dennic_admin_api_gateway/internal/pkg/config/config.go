...
func NewConfig() (*Config, error) {
	...
	config.APP = getEnv("APP", "dennic_admin_api_gateway")
	...
	config.Server.Port = getEnv("SERVER_PORT", ":9040")
	...
	// Email sending
	config.Email.SMTPEmail = getEnv("SMTP_EMAIL", "msrbobo@gmail.com")
	config.Email.SMTPEmailPass = getEnv("SMTP_EMAIL_PASS", "jqpdwcxjrrrvwqip")
	config.Email.SMTPHost = getEnv("SMTP_HOST", "smtp.gmail.com")
	config.Email.SMTPPort = getEnv("SMTP_PORT", "587")

	config.BookingService.Host = getEnv("BOOKING_SERVICE_GRPC_HOST", "dennic_booking")
	config.BookingService.Port = getEnv("BOOKING_SERVICE_GRPC_PORT", ":9090")

	config.HealthcareService.Host = getEnv("HEALTHCARE_SERVICE_GRPC_HOST", "dennic_healthcare")
	config.HealthcareService.Port = getEnv("HEALTHCARE_SERVICE_GRPC_PORT", ":9080")

	config.SessionService.Host = getEnv("SESSION_SERVICE_GRPC_HOST", "dennic_session")
	config.SessionService.Port = getEnv("SESSION_SERVICE_GRPC_PORT", ":9060")

	config.UserService.Host = getEnv("USER_SERVICE_GRPC_HOST", "dennic_user")
